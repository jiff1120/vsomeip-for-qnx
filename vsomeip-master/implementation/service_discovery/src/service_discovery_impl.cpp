...
//            + (cyclic_offer_delay_ / 10);
    last_msg_received_timer_timeout_ = cyclic_offer_delay_ * 2;
}
            ...
            if ((!is_suspended_)
                    ...
                    || (is_diagnosis_
                            && !configuration_->is_someip(its_service.first,
                                    its_instance.first)))) {
                VSOMEIP_ERROR<<"service_discovery_impl::insert_offer_entries 1 "
                            <<"_ignore_phase="<<_ignore_phase
                           <<"is_in_mainphase="<<its_instance.second->is_in_mainphase()
...
service_discovery_impl::send(bool _is_announcing) {
    ...
    if (its_runtime) {
        ...
        if (_is_announcing) {
            ...
            for(auto i : its_offers)
            {
                for(auto j : i.second)
                    VSOMEIP_ERROR<<"service_discovery_impl::send : offer service : "<<std::hex<<j.second->get_service()<<" instance : "<<std::hex<<j.second->get_instance()<<" is local:"<<j.second->is_local();
            }  
            // Serialize and send
...
service_discovery_impl::insert_offer_service(
    ...
    if (its_entry) {
        ...
        add_entry_data(_messages, its_data);
        VSOMEIP_ERROR << __func__ << ": add_entry_data.";
    } else {
...
service_discovery_impl::send(
    ...
    VSOMEIP_ERROR<<"service_discovery_impl::send 1";
    ...
    for (const auto &m : _messages) {
        if (m->has_entry()) {
            ...
            VSOMEIP_ERROR<<"service_discovery_impl::send 2";
