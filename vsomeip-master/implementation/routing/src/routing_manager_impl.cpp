...
//        if_state_running_(false),
        if_state_running_(true), // yinjf, without netlink to get netif state,default to true
        sd_route_set_(false),
...
//#ifndef _WIN32
#if !defined(_WIN32) && !defined(__QNX__) //yinjf
...
bool routing_manager_impl::offer_service(client_t _client,
    ...
    VSOMEIP_ERROR<<__func__<<":insert_offer_command ok";
    ...
    {
        VSOMEIP_ERROR<<__func__<<":init_service_info:"<<if_state_running_;
        std::lock_guard<std::mutex> its_lock(pending_sd_offers_mutex_);
...
bool routing_manager_impl::send(client_t _client, const byte_t *_data,
    ...
    VSOMEIP_ERROR<<"routing_manager_impl::send 11";
    bool is_sent(false);
...
#endif
            VSOMEIP_ERROR<<"routing_manager_impl::send 22";
                    ...
                    if (its_target) {
                        ...
                        if (its_header.prepare(its_target, true, _instance))
                            tc_->trace(its_header.data_, VSOMEIP_TRACE_HEADER_SIZE,
                                    _data, its_data_size);
#endif
                        VSOMEIP_ERROR<<"routing_manager_impl::send 850";
    ...
    if (configuration_) {
        ...
        if (_is_local_service) {
            ...
            if (ILLEGAL_PORT != its_reliable_port) {
                VSOMEIP_ERROR<<__func__ <<" create reliable_endpoint:"<<its_reliable_port;
                std::shared_ptr<endpoint> its_reliable_endpoint  =
                        ep_mgr_impl_->find_or_create_server_endpoint(
                                its_reliable_port, true, is_someip, _service,
                                _instance, _is_found);
                if (its_reliable_endpoint) {
                    VSOMEIP_ERROR<<__func__ <<" create reliable_endpoint:"<<its_reliable_port<<" ok";
            ...
            if (ILLEGAL_PORT != its_unreliable_port) {
                VSOMEIP_ERROR<<__func__ <<" create unreliable_endpoint:"<<its_unreliable_port;
                std::shared_ptr<endpoint> its_unreliable_endpoint =
                        ep_mgr_impl_->find_or_create_server_endpoint(
                                its_unreliable_port, false, is_someip, _service,
                                _instance, _is_found);
                if (its_unreliable_endpoint) {
                    VSOMEIP_ERROR<<__func__ <<" create unreliable_endpoint:"<<its_unreliable_port<<" ok";
